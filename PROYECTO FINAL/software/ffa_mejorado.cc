...
//FUNCION QUE OBTIENE UN HIJO DE DOS LUCIERNAGAS
//elem1 -> primer padre de la reproduccion
//elem2 -> segundo padre de la reproduccion
//poblacion -> poblacion de luciernagas en la que introducire el hijo generado
void reproduccion(Luciernaga elem1, Luciernaga elem2, vector<Luciernaga> &poblacion){

    //Hijo que generaremos e introduciremos en la poblacion
    Luciernaga hijo;
    //Cantidad de elementos del peor padre que combinaremos con el mejor
    int numero_cambios = 0.2 * elem1.solucion.size();

    //Si el padre 'elem1' es mejor que el padre 'elem2'
    if(elem1.fitness<elem2.fitness){
        
        //Entonces el hijo tendra una configuracion base similar al padre 'elem1' 
        hijo = elem1;
        
        //Introducimos del peor padre de los dos la cantidad de elementos arriba calculada en las primeras posiciones
        //(las posiciones serian aleatorias pero por eficiencia no generamos estos, los escogemos del comienzo) 
        for(int i = 0 ; i < numero_cambios; i++){

            hijo.solucion[i] = elem2.solucion[i];

        }

    //Si el padre 'elem2' es mejor que el padre 'elem1'
    }else{

        //Entonces el hijo tendra una configuracion base similar al padre 'elem2' 
        hijo = elem2;
        ...
        for(int i = 0 ; i < numero_cambios; i++){
            ...
            hijo.solucion[i] = elem1.solucion[i];
    ...
    hijo.fitness = cec17_fitness(&hijo.solucion[0]);

    //Introducimos en la ultima posicion de la poblacion (el peor elemento) el hijo generado
...
int main(int argc, char *argv[]) {
    ...
    for (int id_funcion = 1; id_funcion <= 30; id_funcion++) {
        ...
        cec17_init("Firefly_mejorado", id_funcion, dim);
        ...
        //Contador que llevara las iteraciones totales. Estas no son equivalente a las evaluaciones.
        //Una iteracion es una pasada por la poblacion completa acercando unas luciernagas a otras
        int contador = 0;

        //Mientras no se cumpla la condicion de parada asociada a la cantidad de evaluaciones a hacer
        while (evaluaciones < 10000 * dim) {

            //Cada 3 iteraciones (3 vueltas a toda la poblacion acercando luciernagas a otras mas brillantes)
            if(contador > 0 && contador % 3 ==0){

                //Ordenamos la poblacion de mejor a peor usando el operador definido en el Struct de Luciernaga
                //El primer elemento es aquel con el menor fitness y el ultimo el que mayor tiene
                sort(poblacion.begin(), poblacion.end());

                //Representa la distancia entre una luciernaga y otra
                double distancia_entre_elementos = 0.0;
            
                vector<Luciernaga> poblacion_auxiliar;

                //Vamos a ver la distancia entre cada una de las primeras 10 luciernagas (que una vez ordenada
                //la poblacion son las 10 con mejor fitness)
                for(int k = 0; k < 9; k++){

                    //Para no calcular la distancia entre la i - j y la j - i, empezamos por la siguiente en adelante 
                    for(int j = k+1; j < 10; j++){

                        //Para cada par nuevo de luciernagas se reestablece la distancia
                        distancia_entre_elementos = 0.0;

                        //Vamos elemento por elemento de ambas luciernagas que estamos comparados para 
                        //ver la distancia entre ellas
                        for(int i = 0; i < dim; i++){

                            //Se suma la diferencia al cuadrado de cada elemento del vector solucion de ambas
                            //luciernagas
                            distancia_entre_elementos += (poblacion[k].solucion[i]-poblacion[j].solucion[i])*(poblacion[k].solucion[i]-poblacion[j].solucion[i]);

                        }

                        //Se hace la raiz cuadrada a esa suma de cuadrados para obtener la distancia total
                        distancia_entre_elementos = sqrt(distancia_entre_elementos);

                        //Si la distancia entre ellas es pequeña en relacion con la cantidad de elementos 
                        //dentro de cada luciernaga
                        if(distancia_entre_elementos < 10*dim){

                            //Llamamos a la funcion reproduccion que obtendra un hijo de estas luciernas cercanas
                            reproduccion(poblacion[k], poblacion[j], poblacion_auxiliar);
                            //Por cada hijo que generamos tenemos que calcular su fitness, por lo que aumentamos las evaluaciones
                            evaluaciones++;
                ...
                for(int g = 0 ; g < poblacion_auxiliar.size() && g < 10; g++){

                    poblacion[poblacion.size()-1-g] = poblacion_auxiliar[g];
